...
SENTINEL v1 retains structured evidence and audit events indefinitely in the local demo store, while raw source material remains in scenario fixtures or integration-owned storage and is referenced by hash or path. Investigation state does not copy raw logs, traces, Slack payloads, or other unbounded source material.
