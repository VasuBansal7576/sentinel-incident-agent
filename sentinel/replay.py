...
TOOL_IMPLEMENTATIONS: dict[str, dict[str, Any]] = {
    ...
    "infra.spawn_service_investigator": {
        "claim": "Service Investigator returned a scoped report for {service}.",
        "data": {
            "service_report": {
                "service_name": "payment-service",
                "local_diagnosis": (
                    "payment-service symptoms correlate with PR #847 and the "
                ...
                "estimated_user_impact": 18420,
                "evidence_gaps": [],
                "conflicts": [],
                "isolated_context_id": "ctx-deterministic-replay",
                "scoped_tool_names": [
                    "observe.fetch_service_logs",
                    "repo.get_recent_commits",
                    "repo.get_rollback_targets",
    ...
    "infra.scale_replicas": {
        "claim": "{service} replica count changed in the deterministic replay runtime.",
        "data": {"status": "executed", "from_replicas": 6, "to_replicas": 9},
    },
    "infra.toggle_feature_flag": {
        "claim": "Feature flag change was applied in the deterministic replay configuration store.",
        "data": {"status": "executed", "flag": "checkout_fast_path", "enabled": False},
    },
    "infra.add_database_index": {
        "claim": "Database index migration plan for orders.user_id was prepared and applied in deterministic replay.",
        "data": {"status": "executed", "index": "idx_orders_user_id", "estimated_lock_ms": 0},
    },
    "infra.flush_cache": {
        "claim": "{service} cache namespace was flushed in the deterministic replay cache backend.",
    ...
    "infra.modify_env_config": {
        "claim": "Environment configuration change was written to the deterministic replay deployment spec.",
    ...
    "infra.run_migration": {
        "claim": "Migration runner executed the approved migration in the deterministic replay database.",
...
class ReplayIncidentEnvironment:
    ...
    def invoke(self, contract: ToolContract, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if implementation is None:
            raise ToolExecutionError(
                ToolErrorKind.PERMANENT,
                f"{contract.name} has no deterministic replay implementation",
                retryable=False,
            )
        ...
        data = {
            "tool": contract.name,
            "implementation": f"deterministic_replay::{contract.name}",
            "stub": False,
